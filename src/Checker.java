...
public class Checker {
    public static int checker(TTSPData data, TTSPSolution solution) {
        ...
        for (int i = 1; i <= nb_tech; i++) { //browsing the list of technicians
            for (int j = 1; j <= nb_days; j++) { // browsing the list of days

                //We check that the technician i unavailable on day j is assigned to team 0 /////////////////////
                int check = 0;
                for (int k = 1; k <= data.getTechnician()[i].getDispo().length; k++) { // browsing the list of days where technician i is unavailaible
        ...
        for (int i = 0; i < nb_interv; i++) {
            ...
            int ST = solution.getInterv_dates()[i].getInterv();
            int end_i = start_i + data.getIntervention()[ST].getTime();
        ...
        for (int i = 0; i < solution.getInterv_dates().length; i++) {
            ...
            for (int m = 0; m < solution.getTech_teams()[day].getTeam()[team][0]; m++) {
                int tech = solution.getTech_teams()[day].getTeam()[team][m];
                for (int j = 0; j < nb_domain; j++) {
                    int level = data.getTechnician()[tech].getD()[j];
